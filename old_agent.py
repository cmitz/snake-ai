...
class Agent:
    ...
    def route_to_directions(self, current_position, direction, route):
        ...
        print(f"Route: {route}")
        for next_step in route:

            direction_count = len(directions)
            if direction == Direction.NORTH:
                if next_step[1] == current_position[1] - 1:  # Straight ahead
                    directions.append(Move.STRAIGHT)
                elif next_step[0] == current_position[0] + 1:  # To Right
                    directions.append(Move.RIGHT)
                    direction = Direction.EAST
                elif next_step[0] == current_position[0] - 1:  # To LEFT
                    directions.append(Move.LEFT)
                    direction = Direction.WEST

            elif direction == Direction.EAST:
                if next_step[0] == current_position[0] + 1:  # Straight ahead
                    directions.append(Move.STRAIGHT)
                elif next_step[1] == current_position[1] - 1:  # To Right
                    directions.append(Move.RIGHT)
                    direction = Direction.SOUTH
                elif next_step[1] == current_position[1] + 1:  # To Left
                    directions.append(Move.LEFT)
            ...
            if len(directions) == direction_count:
                print("Something went WRONG dude")
            else:
                print(f"Go {directions[len(directions)-1]}")
